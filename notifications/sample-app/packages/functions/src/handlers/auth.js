...
import {registerScriptTag,removeAllScriptTags} from '@functions/services/registerScriptTagServices';
...
app.use(
  shopifyAuth({
    ...
    afterInstall: async ctx => {
      await installationService.installApp(ctx);
      await registerScriptTag(ctx)
      await registerWebhook(ctx)

    },
    afterUninstall:  async ctx => {
      await uninstallationService.uninstallApp(ctx);
      await removeAllScriptTags(ctx)
